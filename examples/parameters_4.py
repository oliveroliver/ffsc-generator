...
outline_corner_radius = 0.3  # on the overall outline, the radius to make the 90 degree corners
outline_angle_radius = 2.0  # on the overall outline, the radius to make the other corners.
...
triangle_height = 3.5  # the total height of each triangle
...
distance_arc_from_edge = 1.2  # the distance of the pad arc start point from the edge of the connector - should really be above the via point
...
inter_pad_spacing = 0.6  # the spacing between the pads - will affect solderability
...
pad_length_to_max_track_ratio = 1.5  # this sets the pad length. This is worked out by taking the max track width, multiplied by pad_width_to_track_ratio, multiplied by this value.

additional_track_from_pad_length = 0.0  # the distance from the pad before starting the convergence of tracks on each end segment
...
segment_length = [10, 8, 6, 9]  # this specifies both the length of each straight segment
segment_angle = [-45, 45, 90]  # this specifies the angle of each join / curved section. There should be one less angle than connections in length list
segment_inner_radius = [2, 2, 2]  # the radius of each angle

# define the width of each track and number of tracks

track_widths = [1.0, 1.0, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25]
